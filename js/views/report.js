...
async function renderReport() {
  ...
  document.getElementById('guardianUploadStatus').innerHTML = gCount > 0
    ? '<span style="color:var(--green-text)">' + t('report.guardians_ok', { n: gCount }) + '</span>'
    : '<span style="color:var(--amber-text)">' + t('report.no_guardians') + '</span>';
  ...
  if (redCards.length === 0 && absentCards.length === 0) {
    html = '<div style="font-size:14px;color:var(--text2);padding:3rem;text-align:center">' + t('report.none', { date: date }) + '</div>';
  } else {
    if (absentCards.length) {
      const n = absentCards.length;
      html += '<div style="font-size:12px;color:var(--text2);text-transform:uppercase;letter-spacing:0.04em;margin-bottom:10px;margin-top:' + (redCards.length ? '1.5rem' : '0') + '">' + t(n > 1 ? 'report.absent_pl' : 'report.absent', { n }) + '</div>';
      ...
      absentCards.sort(function(a, b) { return a.s.cls.localeCompare(b.s.cls) || a.s.name.localeCompare(b.s.name, 'sv'); }).forEach(function(x) {
        html += '<div style="background:var(--surface);border:1px solid #8b5cf6;border-radius:var(--radius-lg);padding:12px;box-shadow:0 2px 8px rgba(139,92,246,0.08)"><div style="display:flex;align-items:center;gap:8px;margin-bottom:6px"><span class="badge" style="background:#8b5cf6;color:#fff">' + t('report.badge_absent') + '</span><span style="font-size:14px;font-weight:600;flex:1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">' + x.s.name + '</span><span style="font-size:11px;color:var(--text3);font-family:monospace;flex-shrink:0">' + x.s.cls + '</span></div>' + guardianBlock(x.logId, students, guardians, barcodeMap) + '</div>';
      });
      html += '</div>';
    }
    if (redCards.length) {
      const n = redCards.length;
      html += '<div style="font-size:12px;color:var(--text2);text-transform:uppercase;letter-spacing:0.04em;margin-bottom:10px">' + t(n > 1 ? 'report.nhi_pl' : 'report.nhi', { n }) + '</div>';
      html += '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:8px;margin-bottom:1rem">';
      redCards.sort(function(a, b) { return a.s.cls.localeCompare(b.s.cls) || a.s.name.localeCompare(b.s.name, 'sv'); }).forEach(function(x) {
        html += '<div style="background:var(--surface);border:1px solid var(--red-border);border-radius:var(--radius-lg);padding:12px;box-shadow:0 2px 8px rgba(229,57,58,0.08)"><div style="display:flex;align-items:center;gap:8px;margin-bottom:6px"><span class="badge badge-out">' + t('report.badge_nhi') + '</span><span style="font-size:14px;font-weight:600;flex:1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">' + x.s.name + '</span><span style="font-size:11px;color:var(--text3);font-family:monospace;flex-shrink:0">' + x.s.cls + '</span></div>' + guardianBlock(x.logId, students, guardians, barcodeMap) + '</div>';
      });
      html += '</div>';
    }
  }
